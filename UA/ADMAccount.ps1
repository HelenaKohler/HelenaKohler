...
Set-ADUser ($UserID) -Description ($Description) -Department ($Kststelle) -Company ($Company) -AccountExpirationDate $Ablauf  -replace @{msnpallowdialin=$false; "extensionattribute3" = $Attribute3; "extensionattribute14" = $Attribute14} -Server $DomainCotroller
